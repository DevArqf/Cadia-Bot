...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('purge')
    .setDescription('Bulk delets given amount of messages. Limit is 100.')
  ...
  async execute(interaction) {
    if(!interaction.member.permissions.has(PermissionsBitField.Flags.ManageMessages)) {
      const noPermissionEmbed = new EmbedBuilder()
        ...
        .setDescription('<:bl_x_mark:1206436599794241576> You do not have the required permissions to execute this command.')
        .setTimestamp();
    ...
    try {
      ...
      .setFooter({ text: `${footer_message}`, iconURL: interaction.client.user.displayAvatarURL() })
        .setColor('#FF0000')
        .setTitle('`❌` Error Purging Messages')
        .setDescription('Uh Oh... Beemo has encountered an error while purging messages.')
        .setTimestamp();
