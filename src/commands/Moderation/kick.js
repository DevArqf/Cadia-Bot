...
class UserCommand extends BeemoCommand {
	...
	async chatInputRun(interaction) {
		...
		if (!kickMember.kickable) {
			return await interaction.reply({
				content: '<:bl_x_mark:1206436599794241576> I **cannot** kick this user because they are either **higher** than me or you!',
				ephemeral: true
			});
		}
		if (interaction.member.id === kickMember.id) {
			return interaction.reply({ content: '<:bl_x_mark:1206436599794241576> You cannot kick yourself!', ephemeral: true });
		}
		if (kickMember.permissions.has(PermissionsBitField.Flags.Administrator)) {
			return interaction.reply({
				content: '<:bl_x_mark:1206436599794241576> You **cannot** kick **staff members** or people with the **Administrator** permission!',
				ephemeral: true
			});
		}
