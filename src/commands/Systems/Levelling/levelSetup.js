...
class UserCommand extends BeemoCommand {
	...
	async chatInputRun(interaction) {
		try {
			...
			if (existingLevel) {
				const defaultMessage = `${emojis.custom.tada} **Congratulations** <@${userId}>! You have **leveled up** to level **${existingLevel.userLevel}**!`;
				await Level.create({
					guildId,
					channelId,
				...
				await interaction.reply({ content: `${emojis.custom.fail} The level system has **already** been setup!`, ephemeral: true });
			}
		} catch (error) {
			console.error(error)
